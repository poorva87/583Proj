...
abstract /*package*/ class MemberListParser {
    ...
    private void parse() {
        ...
        if (observer != null) {
            ...
            if(observer!=null && observer instanceof DotDumper){
            	try {
            		((DotDumper)observer).writerGraph = new BufferedWriter(new FileWriter("Graph.txt"));
            		((DotDumper)observer).writerInstr = new BufferedWriter(new FileWriter("Instructions.txt"));
        ...
        if(observer!=null && observer instanceof DotDumper){
        	...
        	try{
        	((DotDumper)observer).writerGraph.close();
        	((DotDumper)observer).writerInstr.close();
        	}catch(Exception e){
        		e.printStackTrace();
        		System.exit(1);
        	}
        ...
        //if(observer!=null)
        	//observer.resolveAllMethodBBEdges();
