...
public class CfTranslator {
    ...
    //583 Specific variables
    /**583* Indicates whether the dump file is already opened*/
    private static boolean isDumpFileOpen = false;
    /**583* Name of the dumpfile*/
    private static final String dumpFileName = "583Dump.txt";
    private static BufferedWriter dumpFile = null;
    ...
    //Function to dump all the required info to an external file
    private static void dump583Stuff(String className, String methodName, CstMethodRef meth, RopMethod rmeth)
    {
    	if(!isDumpFileOpen)
    	{
    		try {
				dumpFile = new BufferedWriter(new FileWriter(dumpFileName));
				isDumpFileOpen = true;
    	...
    	if(isUDF(className, methodName))
    	{
    		String fullMethodName = getMethodNameWithParams(methodName, meth);
    		String preHeader = String.format("%s\t%s\t", className, fullMethodName);
    		
    		
    	}
    }
    
    private static String getMethodNameWithParams(String methodName, CstMethodRef meth)
    {
    	String params="";
    	Prototype proto = meth.getPrototype();
    	StdTypeList paramTypes = proto.getParameterTypes();
    	...
    	else
    	{
    		...
    		{
    			params+= paramTypes.get(i).toString();
    ...
    //Checks if the function is a user defined function
    private static boolean isUDF(String className, String methodName)
    {
    	return true;
    }
    /**
